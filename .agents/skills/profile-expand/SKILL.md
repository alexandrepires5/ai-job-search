---
name: profile-expand
description: Expand the candidate profile by researching linked public sources, projects, courses, certifications, and portfolio evidence. Use when the user asks to enrich, expand, or deepen their job-search profile.
---

# $profile-expand - Competency Expansion from Documents and Online Presence
...
```
## $profile-expand found [N] new competency signals across [M] sources
...
For each addition, add a brief source annotation in a comment or parenthetical: *(Coursera - Deep Learning Specialisation)*, *(GitHub - project-name)*, etc. This makes future `$profile-expand` runs idempotent.
