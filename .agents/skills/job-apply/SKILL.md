---
name: job-apply
description: Run a complete job application workflow in Codex: evaluate a job posting URL or pasted text, tailor a LaTeX CV, write a cover letter, run reviewer feedback, compile and inspect PDFs, and report final files. Use when the user asks to apply for a role, evaluate and draft an application, tailor CV/cover letter, or invokes $job-apply job workflow.
---

# $job-apply - Drafter-Reviewer Job Application Workflow

You are orchestrating a two-agent job application workflow. The job posting is provided below as `the user's current request` (either a URL or pasted text).
...
- If `the user's current request` looks like a URL, use `WebFetch` to retrieve the job posting content.
- If it is pasted text, use it directly.
- Extract: **company name**, **role title**, **department** (if mentioned), **location/remote policy**, **salary range in EUR** (if listed), and **language** of the posting (Portuguese or English unless another language is clearly required).
...
- **Match the language of the job posting** (Portuguese posting -> Portuguese cover letter, English posting -> English cover letter)
...
Use a Codex subagent for reviewer critique if subagent tools are available; otherwise run a separate self-review pass before revising. The reviewer gets a fresh context, so pass the drafts **inline in the prompt** below (do not make the reviewer Read them). Scope the reviewer's file reads to content-critique essentials only - the reviewer does not need the LaTeX template files (`05`, `06`) to critique content, since those govern structural/LaTeX concerns the drafter already applied.
