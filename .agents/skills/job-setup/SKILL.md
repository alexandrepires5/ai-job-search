---
name: job-setup
description: Onboard or update the candidate profile for the AI job search workspace in Codex. Use when the user asks to set up their profile, import CV/LinkedIn/career documents, run setup, update search configuration, or populate profile files.
---
...
If `the user's current request` contains `--section <name>`, skip directly to that section in Path C for an update-only flow. Do not run the path-selection prompt below.
...
Documents cover skills, experience, education, references, and behavioral signal. They do not cover everything `$job-apply` and `$job-scraper` need. After the writes, ask follow-up questions for gaps:
...
This section generates the search queries that power `$job-scraper`. Use the information from Sections 1, 4, and 7 to build targeted queries.
...
- **Job sources:** "Which sites should I prioritize? For example: LinkedIn, company career pages, Landing.jobs, ITJobs.pt, Wellfound, Otta/Welcome to the Jungle, Indeed, Glassdoor, Net-Empregos, or general web search." Capture Portugal, EU remote, remote Portugal, and target-company preferences.
...
> - `.agents/skills/job-scraper/search-queries.md` - Job search queries for `$job-scraper`
>
> **Try it out:**
> - Run `$job-scraper` to search for matching jobs right now
...
- At the end, suggest running `$job-scraper` and `$job-apply` with a test job posting.
